...
export const YEARS = Array.from({ length: 4 }, (_, i) => String(currentYear - i))
...
export function filterMembersByRange(
  ...
  for (const member of members) {
    ...
    const pledgeEvents = included.filter(
      item => item.type === 'pledge-event' && pledgeEventIds.includes(item.id)
    )
...
export function buildTableRow(member: any, included: any[], dbMember?: any): MemberRow {
  const addressId = member.relationships?.address?.data?.id
  const address = included.find(i => i.type === 'address' && i.id === addressId)
  ...
  return {
    ...
    street: useClean
      ? [dbMember.clean_line_1, dbMember.clean_line_2].filter(Boolean).join(':: ') || '—'
      : [address?.attributes?.line_1, address?.attributes?.line_2].filter(Boolean).join(':: ') || '—',
