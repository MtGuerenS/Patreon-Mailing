...
export function saveMembersCache(campaignId: string, data: any[], included: any[]) {
  store.set('members_cache', JSON.stringify({ campaignId, data, included, cachedAt: Date.now() }))
}

export function loadMembersCache(): { campaignId: string; data: any[]; included: any[]; cachedAt: number } | null {
  try {
    const raw = store.get('members_cache') as string
    if (!raw) return null
...
export async function fetchCampaignMembers(campaignId: string, accessToken: string) {
